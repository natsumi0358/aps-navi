...
> **このファイルは ② APSナビ（本フォルダ）の技術的正本（Single Source of Truth）。**  
> **3本のWebアプリ横断の入口・連携ルール**は親の [`../CLAUDE.md`](../CLAUDE.md) を参照すること。  
> コードを変更したら、このファイルの該当箇所と `SPEC.md` / `README.md` も必ず更新すること。
...
## 引き継ぎログ（最新の実装状態・上が新しい）

Cursor / Claude Code のどちらで作業しても、**コミット前にここへ 1 行追記**する（「何が変わったか」の単一の入口）。

- **2026-04-14** — 引き継ぎログ節を新設。旧ルールの「⑦ 最新の実装状態」参照を本節へ統一。
- **2026-04-14** — DB新フィールド追加（founded/established/headquarters/capital/operating_profit/branches/group_companies/coverage_map）。company_form.htmlをWork6〜13に準拠して全面再設計。HP取得で会社概要サブページ自動検索を実装。「保存のみ」「PPT出力」ボタン分離。aps-advisor-ai/CLAUDE.md・sales-room/README.md・todo.md・情報システム部CLAUDE.mdを含む全体連携ドキュメント整備完了。
...
1. **どちらが作業しても、変更後に本書の「引き継ぎログ（最新の実装状態）」に 1 行追記する**（Cursor は commit 直前、Claude Code は push 直前が目安）
